...
export function DebateArena({ ideas, investigation, onClose }: DebateArenaProps) {
  ...
  const submitFork = useCallback(() => {
    ...
    setForkedArgs((p) => [
      ...p,
      {
        side: forkTarget.side,
        sourceRound: forkTarget.round,
        sourcePoint: forkTarget.point,
        userArgument: forkText.trim(),
      },
    ]);
    setForkTarget(null);
    setForkText("");
      ...
      <div className="flex flex-col items-center gap-4 rounded-xl border border-red-200 bg-red-50 p-8 dark:border-red-800 dark:bg-red-900/20">
        ...
        <div className="flex gap-3">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="rounded-lg border border-neutral-300 px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-100 dark:border-neutral-600 dark:text-neutral-300 dark:hover:bg-neutral-800 transition"
            >
              Close
            </button>
          )}
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl border border-neutral-200 bg-neutral-50 p-8 dark:border-neutral-700 dark:bg-neutral-900">
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400 text-center max-w-md">
          Analyze {ideas.length} idea{ideas.length !== 1 ? "s" : ""} through structured pro/con
          debate rounds.
        </p>
        <button
          onClick={runDebate}
    ...
    <div className="space-y-6">
        ...
        return (
          ...
          >
            <button
              ...
              className="flex w-full items-center justify-between px-4 py-3 text-sm font-medium text-neutral-700 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800 transition"
            >
                ...
                <div className="border-r border-neutral-200 dark:border-neutral-700 bg-green-50/50 dark:bg-green-900/10 p-4 space-y-3">
                  ...
                  {round.proArguments.map((arg, i) => (
                    <ArgCard
                      ...
                      arg={arg}
                      side="pro"
                      k={vk(round.round, "pro", i)}
                      votes={votes[vk(round.round, "pro", i)]}
                      onVote={handleVote}
                      onFork={() =>
                        setForkTarget({ round: round.round, side: "pro", point: arg.point })
                      }
                    />
                  ))}
                  {round.proRebuttal && (
                    <p className="text-xs italic text-green-700 dark:text-green-400 border-t border-green-200 dark:border-green-800 pt-2">
                      Rebuttal: {round.proRebuttal}
                    </p>
                  )}
                </div>
                <div className="bg-red-50/50 dark:bg-red-900/10 p-4 space-y-3">
                  ...
                  {round.conArguments.map((arg, i) => (
                    <ArgCard
                      ...
                      arg={arg}
                      side="con"
                      k={vk(round.round, "con", i)}
                      votes={votes[vk(round.round, "con", i)]}
                      onVote={handleVote}
                      onFork={() =>
                        setForkTarget({ round: round.round, side: "con", point: arg.point })
                      }
                    />
                  ))}
                  {round.conRebuttal && (
                    <p className="text-xs italic text-red-700 dark:text-red-400 border-t border-red-200 dark:border-red-800 pt-2">
                      Rebuttal: {round.conRebuttal}
                    </p>
                  )}
                </div>
              </div>
      ...
      {forkTarget && (
        <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 space-y-3 dark:border-indigo-800 dark:bg-indigo-900/20">
          <p className="text-xs text-indigo-700 dark:text-indigo-300">
            Forking from <span className="font-medium">{forkTarget.side}</span>: &ldquo;
            {forkTarget.point}&rdquo;
          </p>
          <textarea
            value={forkText}
            onChange={(e) => setForkText(e.target.value)}
            placeholder="Add your branching argument…"
            rows={2}
            className="w-full rounded-lg border border-indigo-300 bg-white px-3 py-2 text-sm dark:border-indigo-700 dark:bg-neutral-900 dark:text-neutral-200"
          />
          <div className="flex gap-2">
            <button
              onClick={submitFork}
              disabled={!forkText.trim()}
              className="rounded-lg bg-indigo-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-40 transition"
            >
              Fork
            </button>
            <button
              ...
              className="rounded-lg border border-neutral-300 px-4 py-1.5 text-sm text-neutral-600 hover:bg-neutral-100 dark:border-neutral-600 dark:text-neutral-400 dark:hover:bg-neutral-800 transition"
            >
              Cancel
            </button>
          </div>
      ...
      {forkedArgs.length > 0 && (
        <div className="rounded-xl border border-purple-200 bg-purple-50 p-4 space-y-2 dark:border-purple-800 dark:bg-purple-900/20">
          ...
          {forkedArgs.map((f, i) => (
            <div
              key={i}
              className="rounded-lg border border-purple-200 bg-white p-3 text-sm dark:border-purple-700 dark:bg-neutral-900"
            >
              <span
                className={`inline-block rounded px-1.5 py-0.5 text-xs font-medium mr-2 ${sideBadge(f.side)}`}
              >
                {f.side}
              </span>
      ...
      <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4 space-y-3 dark:border-neutral-700 dark:bg-neutral-900">
        ...
        <div className="flex gap-2">
          <select
            value={customSide}
            onChange={(e) => setCustomSide(e.target.value as "pro" | "con")}
            className="rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-200"
          >
          ...
          <button
            ...
            className="rounded-lg bg-neutral-700 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800 disabled:opacity-40 dark:bg-neutral-600 dark:hover:bg-neutral-500 transition"
          >
            Add
          </button>
        </div>
          ...
          <div className="space-y-1.5">
            {customArgs.map((c, i) => (
              <p key={i} className="text-sm text-neutral-700 dark:text-neutral-300">
                <span
                  className={`inline-block rounded px-1.5 py-0.5 text-xs font-medium mr-2 ${sideBadge(c.side)}`}
                >
                  {c.side}
                </span>
                {c.text}
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-5 space-y-4 dark:border-neutral-700 dark:bg-neutral-900">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`rounded-full px-3 py-0.5 text-xs font-medium ${VERDICT_CLS[result.verdict.winner]}`}
          >
            {result.verdict.winner}
          </span>
          <span className="text-xs text-neutral-500 dark:text-neutral-400">
            {pct(result.verdict.confidence)} confidence
          </span>
        </div>
        ...
        {result.verdict.conditions.length > 0 && (
          <ul className="list-disc pl-5 text-xs text-neutral-600 dark:text-neutral-400 space-y-1">
            {result.verdict.conditions.map((c, i) => (
              <li key={i}>{c}</li>
            ))}
          </ul>
        )}
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-neutral-900">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {(
            [
              ["Depth", result.quality.argumentDepth],
              ["Evidence", result.quality.evidenceQuality],
              ["Balance", result.quality.balanceScore],
              ["Novelty", result.quality.insightNovelty],
...
function ArgCard({
  ...
  k: string;
  votes?: Vote;
  onVote: (key: string, dir: "up" | "down") => void;
  onFork: () => void;
}) {
  const up = votes?.up ?? 0;
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <div className="flex items-center gap-1 ml-auto">
          <button
            onClick={() => onVote(k, "up")}
            aria-label="Upvote"
            className="rounded px-1.5 py-0.5 text-xs hover:bg-green-100 dark:hover:bg-green-900/30 transition"
          >
            ▲ {up}
          </button>
          <button
            onClick={() => onVote(k, "down")}
            aria-label="Downvote"
            className="rounded px-1.5 py-0.5 text-xs hover:bg-red-100 dark:hover:bg-red-900/30 transition"
          >
            ▼ {down}
          </button>
          <button
            onClick={onFork}
            aria-label="Fork argument"
            className="rounded px-1.5 py-0.5 text-xs text-indigo-600 hover:bg-indigo-100 dark:text-indigo-400 dark:hover:bg-indigo-900/30 transition"
          >
            ⑂
          </button>
        </div>
