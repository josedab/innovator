...
export const InnovationIdeaSchema = z.object({
  title: z.string().max(500).describe("Short descriptive title for the innovation idea"),
  description: z.string().max(5000).describe("Detailed explanation of the idea and how it works"),
  potentialImpact: z.string().max(2000).describe("Expected impact if the idea is implemented"),
  implementationHint: z
    .string()
    .max(2000)
    .describe("Practical hint or first step for implementation"),
});

/** Zod schema for the result of applying a single innovation angle. */
export const AngleResultSchema = z.object({
  angleId: z.string().max(100).describe("Identifier of the innovation angle used"),
  angleName: z.string().max(200).describe("Human-readable name of the innovation angle"),
  ideas: z.array(InnovationIdeaSchema).max(50).describe("Innovation ideas generated by this angle"),
...
export const SynthesisSchema = z.object({
  topIdeas: z
    .array(
      z.object({
        title: z.string().max(500).describe("Title of the top-ranked idea"),
        description: z.string().max(5000).describe("Full description of the idea"),
        sourceAngle: z.string().max(200).describe("Innovation angle that produced this idea"),
        potentialImpact: z.string().max(2000).describe("Expected impact of the idea"),
        feasibility: z
          .enum(["low", "medium", "high"])
          .describe("Implementation feasibility rating"),
      })
    )
    .max(50)
    .describe("Top-ranked ideas selected across all angles"),
...
export const CustomAngleSchema = z.object({
  id: z
    ...
    .describe("Unique identifier for the custom angle (lowercase alphanumeric with hyphens)"),
  name: z.string().min(1).max(200).describe("Human-readable display name"),
  description: z.string().min(1).max(2000).describe("Brief description of the angle's approach"),
  promptTemplate: z
    .string()
    .min(1)
    .max(10000)
    .describe("Prompt template with {{subject}} and {{investigation}} placeholders"),
  icon: z.string().max(10).optional().describe("Emoji icon for UI display"),
  author: z.string().max(200).optional().describe("Author of the custom angle"),
  version: z.string().max(50).optional().describe("Semver version string"),
  tags: z
    .array(z.string().max(100))
    .max(20)
    .optional()
    .describe("Categorization tags for discovery and filtering"),
});

/** Schema for an angle pack file (.angle.json). */
export const AnglePackSchema = z.object({
  name: z.string().min(1).max(200).describe("Name of the angle pack"),
  description: z
    .string()
    .max(2000)
    .optional()
    .describe("Description of what angles this pack contains"),
  version: z.string().max(50).optional().describe("Semver version of the angle pack"),
  angles: z
    .array(CustomAngleSchema)
    .min(1)
    .max(50)
    .describe("Array of custom angle definitions included in this pack"),
});
