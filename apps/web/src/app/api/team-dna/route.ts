...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.info("Team DNA analyzed", { route: "/api/team-dna", requestId, durationMs: Date.now() - startTime });
    return Response.json(result, { headers: API_RESPONSE_HEADERS });
  } catch (err) {
    logger.error("Team DNA error", { error: err instanceof Error ? err.message : String(err), route: "/api/team-dna", requestId, durationMs: Date.now() - startTime });
    return new Response(JSON.stringify({ error: "Team DNA analysis failed." }), { status: 500, headers: API_RESPONSE_HEADERS });
  }
}
