...
export function computeEntryContentHash(entry: Omit<LedgerEntry, "contentHash" | "previousHash">): string {
  const content = [
    entry.id,
...
function computeChainHash(previousHash: string, contentHash: string): string {
  return createHash("sha256").update(previousHash + contentHash).digest("hex");
}
...
export function appendEntry(
  ...
  const previousHash =
    sequenceNumber > 0 ? ledger.entries[sequenceNumber - 1].contentHash : "";
...
export function getActorEntries(
  actor: string,
  config: LedgerConfig = {}
): LedgerEntry[] {
  const ledger = loadLedger(config);
...
export function exportForActor(
  actor: string,
  config: LedgerConfig = {}
): GdprExport {
