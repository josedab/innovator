...
export async function POST(request: Request) {
  ...
  try {
    ...
    switch (action) {
      case "list-personas": {
        return new Response(JSON.stringify({ personas: listPersonas() }), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "evaluate": {
        if (!idea || !personaIds?.length) {
          return new Response(JSON.stringify({ error: "idea and personaIds required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        }
        const scorecards = await evaluateWithMultiplePersonas(idea.title, personaIds, { model });
        return new Response(JSON.stringify({ scorecards }), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "assess": {
        if (!idea || !personaIds?.length) {
          return new Response(JSON.stringify({ error: "idea and personaIds required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        ...
        return new Response(JSON.stringify(assessment), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "alignment": {
        if (!ideas?.length || !personaIds?.length) {
          return new Response(JSON.stringify({ error: "ideas and personaIds required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        }
        const matrix = await buildAlignmentMatrix(ideas.map(i => i.title), personaIds, { model });
