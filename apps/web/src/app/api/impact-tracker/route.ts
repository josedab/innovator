...
const ActionSchema = z.object({
  action: z.enum(["track", "update-status", "link-pr", "link-issue", "record-outcome", "impact-score", "rank", "funnel", "dashboard"]),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    switch (action) {
      case "track": {
        if (!idea) return new Response(JSON.stringify({ error: "idea required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        const tracked = trackImpactIdea({ ...idea, status: "proposed", createdAt: new Date().toISOString(), linkedPRs: [], linkedIssues: [], customOutcomes: [], tags: idea.tags ?? [] });
        return new Response(JSON.stringify(tracked), { status: 201, headers: API_RESPONSE_HEADERS });
      }
      case "update-status": {
        if (!ideaId || !status) return new Response(JSON.stringify({ error: "ideaId and status required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        const updated = updateIdeaStatus(ideaId, status);
        return new Response(JSON.stringify(updated), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "link-pr": {
        if (!ideaId || !prUrl) return new Response(JSON.stringify({ error: "ideaId and prUrl required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        const linked = linkPR(ideaId, prUrl);
        return new Response(JSON.stringify(linked), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "link-issue": {
        if (!ideaId || !issueUrl) return new Response(JSON.stringify({ error: "ideaId and issueUrl required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        const linked = linkIssue(ideaId, issueUrl);
        return new Response(JSON.stringify(linked), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "record-outcome": {
        if (!outcome) return new Response(JSON.stringify({ error: "outcome required" }), { status: 400, headers: API_RESPONSE_HEADERS });
        const recorded = recordImpactOutcome({ ...outcome, detectedAt: new Date().toISOString(), metadata: {} });
        return new Response(JSON.stringify(recorded), { status: 201, headers: API_RESPONSE_HEADERS });
      }
      case "impact-score": {
        if (!ideaId) return new Response(JSON.stringify({ error: "ideaId required" }), { status: 400, headers: API_RESPONSE_HEADERS });
      ...
      case "rank": {
        ...
        return new Response(JSON.stringify({ ideas: ranked }), { status: 200, headers: API_RESPONSE_HEADERS });
      }
      case "funnel": {
...
export async function GET(request: Request) {
  try {
    ...
    if (ideaId) {
      ...
      if (!idea) {
        return new Response(JSON.stringify({ error: "Idea not found" }), { status: 404, headers: API_RESPONSE_HEADERS });
      }
      const outcomes = getImpactOutcomes(ideaId);
      return new Response(JSON.stringify({ idea, outcomes }), { status: 200, headers: API_RESPONSE_HEADERS });
    }
