...
const sampleInvestigation = {
  summary: "AI and machine learning are transforming healthcare. Google and OpenAI lead emerging research.",
  currentState: "NLP models like GPT enable clinical text analysis. Kubernetes powers scalable infrastructure.",
  keyAspects: [
...
describe("entity-extractor", () => {
  ...
  describe("extractFromInvestigation", () => {
    ...
    it("extracts technology entities from known patterns", () => {
      const { entities } = extractor.extractFromInvestigation(sampleInvestigation, "session-1");
      const techNames = entities.filter((e) => e.type === "technology").map((e) => e.name.toLowerCase());
      expect(techNames.some((n) => n.includes("ai") || n.includes("nlp") || n.includes("gpt") || n.includes("kubernetes"))).toBe(true);
    });
    ...
    it("builds relationships between extracted entities", () => {
      const { relationships } = extractor.extractFromInvestigation(sampleInvestigation, "session-1");
  ...
  describe("extractFromIdeas", () => {
    ...
    it("extracts technology entities from ideas", () => {
      const { entities } = extractor.extractFromIdeas(sampleIdeas, "session-2");
      const techNames = entities.filter((e) => e.type === "technology").map((e) => e.name.toLowerCase());
      expect(techNames.some((n) => n.includes("gpt") || n.includes("nlp") || n.includes("react") || n.includes("blockchain"))).toBe(true);
    });
  ...
  describe("deduplicateEntities", () => {
    it("merges entities with same normalized stem", () => {
      const entities = [
        { id: "1", name: "Machine Learning", type: "technology" as const, mentions: 2, sessions: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-02", embedding: [] },
        { id: "2", name: "machine learning", type: "technology" as const, mentions: 3, sessions: ["s2"], firstSeen: "2024-01-03", lastSeen: "2024-01-04", embedding: [] },
      ];
    ...
    it("keeps distinct entities separate", () => {
      const entities = [
        { id: "1", name: "AI", type: "technology" as const, mentions: 1, sessions: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", embedding: [] },
        { id: "2", name: "Blockchain", type: "technology" as const, mentions: 1, sessions: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", embedding: [] },
      ];
      const result = extractor.deduplicateEntities(entities);
  ...
  describe("buildSubgraph", () => {
    it("returns neighborhood of a given entity", () => {
      const graph: KnowledgeGraph = {
        nodes: [
          { id: "a", label: "Node A", type: "concept", sourceSessionIds: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", occurrenceCount: 1 },
          { id: "b", label: "Node B", type: "technology", sourceSessionIds: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", occurrenceCount: 2 },
          { id: "c", label: "Node C", type: "domain", sourceSessionIds: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", occurrenceCount: 1 },
          { id: "d", label: "Node D", type: "concept", sourceSessionIds: ["s1"], firstSeen: "2024-01-01", lastSeen: "2024-01-01", occurrenceCount: 1 },
        ],
        edges: [
          { id: "e1", source: "a", target: "b", type: "related_to", weight: 0.8, sourceSessionIds: ["s1"] },
          { id: "e2", source: "b", target: "c", type: "enables", weight: 0.6, sourceSessionIds: ["s1"] },
          { id: "e3", source: "c", target: "d", type: "related_to", weight: 0.5, sourceSessionIds: ["s1"] },
        ],
