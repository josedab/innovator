...
async function POST(request: Request) {
  try {
    ...
    if (body.action === "contribute") {
      const patterns = mockExtractPatterns({ nodeId: "node-1", domain: body.domain, angleResults: body.angleResults });
...
describe("GET /api/genome", () => {
  ...
  it("returns insights when view=insights", async () => {
    mockGenerateGenomeInsights.mockReturnValue([{ type: "angle-recommendation", content: "Use SCAMPER", confidence: 0.8 }]);
...
describe("POST /api/genome", () => {
  ...
  it("contributes patterns", async () => {
    ...
    const res = await POST(makePostRequest({
      action: "contribute",
      domain: "fintech",
      angleResults: [{ angleId: "scamper", angleName: "SCAMPER", ideasCount: 5, successRate: 0.8 }],
    }));
  ...
  it("enriches angle selection", async () => {
    mockEnrichAngleSelection.mockReturnValue({ angles: ["scamper", "first-principles"], enrichments: ["Added first-principles"], insightCount: 3 });
    const res = await POST(makePostRequest({
      action: "enrich",
      angles: ["scamper"],
      domainHint: "fintech",
