...
function makeRequest(body: unknown): Request {
  return new Request("http://localhost/api/novelty", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
describe("POST /api/novelty", () => {
  ...
  it("assesses novelty of ideas", async () => {
    ...
    const res = await POST(
      makeRequest({ ideas: [{ title: "Test Idea", description: "A novel approach" }] })
    );
  ...
  it("returns markdown format when requested", async () => {
    ...
    const res = await POST(
      makeRequest({ ideas: [{ title: "Test", description: "Desc" }], format: "markdown" })
    );
  ...
  it("seeds prior art", async () => {
    mockGetPriorArtCount.mockReturnValue({
      total: 1,
      patents: 1,
      papers: 0,
      products: 0,
    ...
    const res = await POST(
      makeRequest({
        action: "seed",
        entries: [
          { id: "p1", source: "patent", title: "Test Patent", description: "Desc", similarity: 0 },
        ],
      })
    );
    expect(res.status).toBe(200);
